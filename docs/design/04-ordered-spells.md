...
Status: ordered recipes, the shared mana pool, and seven classic spells implemented and verified in-engine (2026-09-11). Invoking resolves the exact orb sequence against the 27-entry table; weapon recipes equip a hand; Ghost Walk (Q,Q,W), Sunstrike (E,E,E), EMP (W,W,W), Chaos Meteor (W,E,E), Tornado (Q,W,W), Deafening Blast (Q,W,E), and Cold Snap (Q,Q,Q) equip a hand and cast on that hand's fire button, spending mana and starting their cooldown; the three remaining spells (Ice Wall, Alacrity, Forge Spirit), the portal, and reserved entries report "not castable yet" without granting anything. The remaining three spell behaviors are not implemented yet. This target supersedes the order-independent recipe target in `01-invoker-mechanics.md`; that document and `05-dual-hands.md` describe the earlier slices.
...
|-------|--------------|----------------|
| Cold Snap | Aim at an enemy to apply a debuff. Subsequent qualifying damage causes brief freezes and additional damage, with an internal trigger cooldown. The built Cold Snap follows this spec: a 1000-unit aim; a 5 s debuff; any damage except the trigger's own freezes the chilled player's movement for 250 ms and adds 15 damage, at most once per 900 ms. | Repeated hits trigger at the allowed interval. Its own damage cannot recurse into another trigger. |
...
Implementation is complete only when the applicable items below pass. Progress (2026-09-11): ordered recipes and the two-slot weapon path pass on the host and in-engine; the mana system, Ghost Walk, Sunstrike, EMP, Chaos Meteor, Tornado, Deafening Blast, and Cold Snap pass their in-engine exercise (Cold Snap's aim cast is exercised in-engine; its hit reaction rides the shared-state host tests, with a live two-player check still open); the remaining three spell rows and the portal are still open.
