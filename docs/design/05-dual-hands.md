...
HUD feedback (branch feat/invoker-hud): each orb ring carries its key letter, so the D/W/A mapping to Q/W/E is readable on the ring itself. A recharging spell shows a full amber bar that drains as the spell comes back, and its label dims while recharging; the bar sits on its own row under the hand labels. The clock is keyed to the spell, matching the server's per-spell cooldown: two hands holding the same spell share one readout, and swapping a hand to another spell never shows a stale bar. The server confirms each successful cast with an `invcast` message; the client draws the timer from that, so a rejected cast never starts the readout.

Second spell batch (branch feat/spells-2): EMP (W,W,W) charges for 2.5 s at the cast point, then bursts for 70 damage in a 600-unit radius and shoves everyone away from the center; the caster is immune. The charge ring is drawn from cg state after an `invemp` announcement. Chaos Meteor (W,E,E) launches a slow, heavy missile (500 units per second, 80 damage, 160-unit splash) that keeps the rocket's trail and explosion visuals; it can hurt the caster, so distance matters. A cast that fails for mana now says so with a centerprint instead of staying silent, and both new spells feed the same recharge readout as Ghost Walk and Sunstrike.
